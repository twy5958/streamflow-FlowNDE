...
def _natural_cubic_spline_coeffs_without_missing_values(times, path):
    ...
    if length < 2:
        ...
        system_rhs[..., -1] = 0
        system_rhs[..., 1:] += path_diffs_scaled
        knot_derivatives = controldiffeq.misc.tridiagonal_solve(system_rhs, time_diffs_reciprocal, system_diagonal,
...
class NaturalCubicSpline:
    ...
    def derivative(self, t):
        ...
        return deriv
    '''
    def derivative_sequence(self, t_sequence):
    """
    Evaluates the derivative of the natural cubic spline at a sequence of points t_sequence.
    t_sequence: A tensor or list of scalar tensors representing the time points.
    Returns:
        A tensor containing the derivative evaluated at each point in t_sequence.
    """
        fractional_parts, indices = self._interpret_t(t_sequence)
        # Compute inner terms for all points in t_sequence
        inner = self._two_c[..., indices, :] + self._three_d[..., indices, :] * fractional_parts
        # Compute derivatives for all points in t_sequence
        deriv = self._b[..., indices, :] + inner * fractional_parts
        return deriv
    '''
