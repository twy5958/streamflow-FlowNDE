...
from ddeat import DDEBlock
#from interpolate import NaturalCubicSpline
import controldiffeq
from lib.dataset import get_normalized_adj_tensor
...
class Flownde(nn.Module):
    def __init__(self, adj, num_node, in_dim, hidden_dim, out_dim, step_size, back=2, thres=0.1, extra=None):
        ...
        self.get_control = nn.Linear((out_dim - 1) * in_dim * 4, (out_dim - 1) * in_dim * 4)#线性层，用于获取控制参数
        self.print = True 
    ...
    def forward(self, A, all_delay, coeffs):
        ...
        spline = controldiffeq.NaturalCubicSpline(times, fake_control)
